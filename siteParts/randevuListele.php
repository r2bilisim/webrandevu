<?php 
	...
	*/
	...
	{	
		...
		{
			case "Mysql":
			$biletler = $db->query("SELECT BILET_NO FROM BILETLER WHERE date(SIS_TAR)='$tarih'", PDO::FETCH_ASSOC);
			break;
			case "Mssql":			
	...
	<div class="row">
		...
		<div class="col-md-4 col-xs-4 dolubos" >
			<input type="radio" disabled id="dolu"/>
			<label for="dolu">
				<p class="pRandevu">Dolu Olan Saatler</p>
			</label>
		</div>&nbsp;
		<div class="col-md-4 col-xs-4 dolubos" >
			<input type="radio" checked id="bos" />
			<label for="bos">
				<p class="pRandevu">Boş Olan Saatler</p>
			</label>
		</div>
		<div class="col-md-2 col-xs-2"></div>
	</div>
				...
				for($i=$basDak; $i<=$sure; $i+=$ara) { 
					...
					if(intval($mevcutTarih) == intval($RandevuTarihi)){
						if(intval($biletSaati)>intval($mevcutSaat)){
							//aynı gün için randevu verilmek isteniyorsa							
						?>
						...
						<div>
							...
							<label for="control_OOnce<?php echo $i; ?>">
								<p class="pRandevu"><?php echo "<br>".$basSaat1.":".$basDak1; ?></p>
	...
	<div align="center" style="position: fixed; z-index:99; left: 0; bottom: 0; width: 100%;background-color:#eee; color: white; text-align: center;">
		<button class="img-responsive button yesil" id="ok" onClick="kaydet(); ">
			<i class="icon-ok" style="font-size: 30px"></i> Bilgileri Onaylıyorum Randevuyu Kaydet
		</button>
		<a href="oturumKapat.php?doLogout=true" class="img-responsive button kirmizi" ><strong>Vazgeç </strong><i class="icon-remove" style="font-size:30px;"></i></a>
	</div>
	<!--RandevuButonu-->
	...
	{
	?>
	<div class="alert alert-danger"><strong>Lütfen bilgilerinizi kontrol ederek yeniden deneyiniz.</strong></div>
	<?php
	} 
?>
