...
<?php
	...
	include($_SERVER['DOCUMENT_ROOT']."/fonksiyonlar/php/dilAyar.php");
		...
		$MusteriAdiSoyadi = $musteriAd." ".$musteriSoyad;
				...
				{		
					...
					#Kuyruk tablosuna ekle		
						...
						{
							case "Mysql":
							$bid = $db -> query("SELECT MAX(BID) FROM BILETLER")->fetch();
							break;
							case "Mssql":
					...
					$durum=true;
					$mailAt=false;
					$icerik="<tr><td colspan='2'><h2>Randevunuz Kaydedildi<i class='icon-ok'></i></h2></td></tr><tr><td><h3>Servis Adı:</h3></td><td><h3 class='btn-primary form-control'>".$gruplar['GRUP_ISMI']."</h3></td></tr><tr><td><h3>Randevu Tarihi:</h3></td><td><h3 class='btn-warning form-control'>".$RandevuTarihiMesaj."</h3></td></tr><tr><td><h3>Bilet No:</h3></td><td><h3 class='btn-danger form-control'><strong>".$biletNo."</strong></h3></td></tr>";
					...
					//$mesaj= "<table id='pdfTable' class='table'>$icerik</table><input type='hidden' name='grp_ismi' value='$gruplar[GRUP_ISMI]' /><input type='hidden' name='adSoyad' value='$musteriAd $musteriSoyad' /><input type='hidden' name='RandevuTarihiMesaj' value='$RandevuTarihiMesaj' /><input type='hidden' name='biletNo' value='$biletNo' /><div><h3>Randevu günü ve saatini kaydetmeyi unutmayınız.</h3><br><button class='btn btn-warning' id='indir'>İndir/Yazdır</button></div>";
					$db->commit();//işlem başarılı ise vtye işlet 
					// action='fonksiyonlar/php/pdfYazdir.php' yenine action='PDF/make/pdfOlustur.php' kullanılırsa eski sistem çalışır
				}
			}
			else
			{
