...
-- Description:	Kişi(tc) Bilgilerini(kontrol.php den) Ajax(tcDogrula.php) ile doğrulamak için yazıldı
-- ============================================= 
 */
...
<!--===============================================================================================-->
	...
	<?php include_once("fonksiyonlar/js/tcdogrula.php");?>
</head>
<body>
<?php if(isset($db)){ //bağlantı ok ?>
<noscript>
...
$row_Gruplar=$db->query("SELECT GRPID, GRUP_ISMI FROM GRUPLAR WHERE Webrandevu=1 AND AKTIF=1")->fetchAll();
		...
		<div class="container-login100">
			...
			<option value="0"><?php echo $diller['servisSec'];?></option>
				...
				<?php } ?>
					...
					<?php if(isset($_POST['GRPID']) && $_POST['GRPID']!=0){ ?>  
						...
						maxlength="11" 
						type="text" 
						placeholder="<?php echo $diller['tc'];?>"
		...
		<tr>
			<td class="alert alert-info"><?php echo $diller['guvenlikKodu'];?>:
		...
		<input type="text" maxlength="5" id="dogrulamaKodu" name="dogrulamaKodu"  tabindex="3" class="form-control" placeholder="<?php echo $diller['guvenlikKoduGir'];?>"  autocomplete="off"  >
			</td>
		</tr>
		...
		<?php }?>
			<div class="container-login100-form-btn">
						<button type="button" class="login100-form-btn btn btn-primary form-control" onclick="dogrula();" name="Gonder"  tabindex="4" <?php if(empty($_POST['GRPID']) || $_POST['GRPID']==0){ echo "disabled"; } ?>><?php echo $diller['devamEt']; ?></button>
					</div>
		<?php } ?>
...
<span class="help-block alert-danger" id="mesaj"></span>
				...
				<?php } else{ ?>
					<tr><td colspan="2"><div class="alert alert-danger"><?php echo $diller['sistemKapaliMesaj']; ?></div></td></tr>
		<?php } ?>
	<div class="container">
